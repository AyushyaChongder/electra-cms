...
function CMSServicesDesignConsulting() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        "https://tn962s9r5i.execute-api.ap-south-1.amazonaws.com/v1/getService?service=design-consulting-service"
      );
  ...
  const handleDeleteClick = async (sectionId) => {
    ...
    try {
      const response = await fetch(
        "https://x5ajuag7dh.execute-api.ap-south-1.amazonaws.com/v1/deleteService?service=design-consulting-service",
        {
          method: "DELETE",
          headers: {
      ...
      for (const image of formData.images) {
        ...
        if (image.file instanceof Blob) {
          ...
          const uploadImageResponse = await fetch(
            `https://7s6x697217.execute-api.ap-south-1.amazonaws.com/v1/uploadNewServiceImage?service=design-consulting-service&subfolder=${sanitizedHeading}`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
