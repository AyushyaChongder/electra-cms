...
function CMSServicesMaintainance() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://tn962s9r5i.execute-api.ap-south-1.amazonaws.com/v1/getService?service=maintenance-repair-service');
  ...
  const handleDeleteClick = async (sectionId) => {
    ...
    try {
      const response = await fetch('https://x5ajuag7dh.execute-api.ap-south-1.amazonaws.com/v1/deleteService?service=maintenance-repair-service', {
        method: 'DELETE',
        headers: {
  ...
  try {
      ...
      for (const image of formData.images) {
          ...
          if (image.file instanceof Blob) {
              ...
              const uploadImageResponse = await fetch(`https://7s6x697217.execute-api.ap-south-1.amazonaws.com/v1/uploadNewServiceImage?service=maintenance-repair-service&subfolder=${sanitizedHeading}`, {
                  method: "POST",
                  headers: {
                      "Content-Type": "application/json",
                  },
     ...
     const updatedAt = new Date().toISOString().slice(0, 19); // Format to "YYYY-MM-DDTHH:mm:ss"
      ...
      const apiEndpoint = isEditing
          ? "https://8jhizf6e46.execute-api.ap-south-1.amazonaws.com/v1/updateService?service=maintenance-repair-service"
          : "https://y1kk288ltd.execute-api.ap-south-1.amazonaws.com/v1/createService?service=maintenance-repair-service";
