...
function CMSServicesApprovals() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        "https://tn962s9r5i.execute-api.ap-south-1.amazonaws.com/v1/getService?service=approval-compliance-service"
      );
  ...
  const handleDeleteClick = async (sectionId) => {
    ...
    try {
      const response = await fetch(
        "https://x5ajuag7dh.execute-api.ap-south-1.amazonaws.com/v1/deleteService?service=approval-compliance-service",
        {
          method: "DELETE",
          headers: {
      ...
      for (const image of formData.images) {
        ...
        if (image.file instanceof Blob) {
          ...
          const uploadImageResponse = await fetch(
            `https://7s6x697217.execute-api.ap-south-1.amazonaws.com/v1/uploadNewServiceImage?service=approval-compliance-service&subfolder=${sanitizedHeading}`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
